...
const interleavedCommunityMedia = (metadata) => {
    return (metadata.community_images || [])
        .filter((image) => image?.url)
        .map((image) => ({ kind: "image", source: "Steam", image }));
};
const steamCommunityItemsFromImages = (appId, title, images) => images
    .filter((image) => image?.url)
    .map((image, index) => {
    const providerIconUrl = playhubCommunityProviderIcon("Steam");
    const itemLink = image.link || image.url;
    const publishedFileId = String(image.id || playhubCommunityId(appId, index));
    const author = image.author || "Steam Community";
    return {
        appid: appId,
        ...
        type: 5,
        title: image.caption || title || "Community item",
        description: image.caption || title || "",
        ...
        creator: playhubCommunityCreator(author, providerIconUrl),
        author,
...
const steamCommunityItemsFromMetadata = (appId, metadata) => steamCommunityItemsFromImages(appId, metadata.title || "Community item", interleavedCommunityMedia(metadata).map((item) => item.image));
...
const communityHubPageForApp = async (appId, page = 1) => {
    const overview = getOverview(appId);
    if (!appId || !isNonSteamApp(overview))
        return null;
    await ensureMetadataCache();
    ...
    if (cleanPage === 1 && metadata?.community_images?.length) {
        return { hub: steamCommunityItemsFromMetadata(appId, metadata) };
    }
    try {
        const { items } = await getSteamCommunityPage(appId, cleanPage);
        const hub = steamCommunityItemsFromImages(appId, metadata?.title || appName(appId) || "Community item", (items || []));
        return { hub };
    }
    catch (error) {
        warn("bridge", "community page fetch failed", error);
        return { hub: [] };
    }
};
        ...
        const patchFeedMethod = (methodName) => {
            ...
            unpatchers.push(patchMethod(httpClient, methodName, (_thisValue, original, args) => {
                ...
                if (match) {
                    ...
                    const requestParams = args[1]?.params || {};
