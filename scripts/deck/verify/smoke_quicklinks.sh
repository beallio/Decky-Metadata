...
#
#   smoke_quicklinks.sh <matched-appid> <never-on-steam-appid>
#
...
python3 - "$matched_json" "$never_json" <<'PY' || exit 1
import json, sys
matched, never = json.loads(sys.argv[1]), json.loads(sys.argv[2])
...
PY
pass "quick-links: matched=$matched shows row, never-on-steam=$never suppressed, metadata intact"
