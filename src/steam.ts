...
const interleavedCommunityMedia = (metadata: MetadataData) => {
  return (metadata.community_images || [])
    .filter((image) => image?.url)
    .map((image) => ({ kind: "image" as const, source: "Steam", image }));
};

const steamCommunityItemsFromImages = (
  appId: number,
  title: string,
  images: NonNullable<MetadataData["community_images"]>
) =>
  images
    .filter((image) => image?.url)
    .map((image, index) => {
...
const steamCommunityItemsFromMetadata = (appId: number, metadata: MetadataData) =>
  steamCommunityItemsFromImages(
    appId,
    metadata.title || "Community item",
    interleavedCommunityMedia(metadata).map((item) => item.image)
  );
...
const communityHubPageForApp = async (appId: number, page = 1) => {
  ...
  try {
    ...
    const hub = steamCommunityItemsFromImages(
      appId,
      metadata?.title || appName(appId) || "Community item",
      (items || []) as NonNullable<MetadataData["community_images"]>
    );
    return { hub };
  } catch (error) {
    log.warn("bridge", "community page fetch failed", error);
    return { hub: [] };
  }
};
