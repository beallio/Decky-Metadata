...
def test_sanitization_classifies_existing_delisted_records(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(plugin, "_load_delisted_index_sync", fake_index)
...
def test_sanitization_does_not_call_ensure_delisted_index_sync(tmp_path, monkeypatch) -> None:
    plugin = make_plugin(tmp_path, monkeypatch)
